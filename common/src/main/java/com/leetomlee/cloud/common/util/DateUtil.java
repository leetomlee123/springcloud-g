package com.leetomlee.cloud.common.util;
...
public class  DateUtil {
    ...
    private final static String MONTH_FORMAT_CN = "yyyy年MM月";
    /**
     *
     */
    ...
    /**
     * Han yaoyang added tools
     * begin
     */
    //传入日期字符串（2018-11-11）返回Date类型日期
    public static final Date dateStringToTime(String dateString){
        return Date.from(LocalDate.parse(dateString).atStartOfDay().atZone(ZoneId.systemDefault()).toInstant());
    }

    //传入日期字符串（2018-11-11）返回n天前或后Date类型日期
    public static final Date dateStringToOtherDate(String dateString, int num){
        return Date.from(LocalDate.parse(dateString).minusDays(num).atStartOfDay()
     ...
     * @see #getFormatDate(java.util.Date, String)
     */
    public static String getFormatDateSFromDate(java.util.Date currDate) {
        return getFormatDate(currDate, DATE_FORMAT_S);
    }
    ...
    public static String getFormatMonthCN(java.util.Date currDate) {
        return getFormatDate(currDate, MONTH_FORMAT_CN);
    }
    /**
